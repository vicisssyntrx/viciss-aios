...
type SettingsTab = "rabbit" | "theme" | "settings";
...
export default function AccountCenter({ onClose, isEmbedded = false }: Props) {
  ...
  const avatarFileInputId = "avatar-file-input-ac";
  ...
  const [activeTab, setActiveTab] = useState<SettingsTab>(() => {
    return (localStorage.getItem("account-settings-tab") as SettingsTab) || "rabbit";
  });
  const handleTabChange = (tab: SettingsTab) => {
    setActiveTab(tab);
    localStorage.setItem("account-settings-tab", tab);
  };
  ...
  // ── Avatar Upload — label-wrapping pattern for reliable mobile support ──
  const handleAvatarPicked = async (file?: File) => {
    ...
    try {
      ...
      toast.error("Failed to upload avatar: " + (error as { message?: string })?.message, { id: "avatar-upload" });
    }
  };
  ...
  const renderRabbitTab = () => (
    <div className="space-y-4 animate-in fade-in duration-200">
      {/* Virtual Assistant Toggle */}
      <div className="glass w-full rounded-2xl p-4 flex flex-col space-y-3">
      ...
      <div className="glass w-full rounded-2xl p-4 flex flex-col space-y-3">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">AI Provider</span>
  ...
  const renderThemeTab = () => (
    <div className="glass w-full rounded-2xl p-4 flex flex-col space-y-4 animate-in fade-in duration-200">
      {/* Theme Mode */}
      ...
      <div className="flex items-center justify-between border-t border-border/30 pt-3">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Primary Color</span>
        <div className="flex items-center gap-2 relative">
          <div 
            className="w-6 h-6 rounded-full border border-border shadow-inner" 
            style={{ backgroundColor: customPrimaryColor }} 
          />
          <label htmlFor="primary-color-picker" className="w-8 h-8 cursor-pointer rounded-md overflow-hidden">
            <input 
  ...
  const renderSettingsTab = () => (
    <div className="glass w-full rounded-2xl p-4 flex flex-col space-y-3 animate-in fade-in duration-200">
      {/* Bulk Import Quotes */}
      ...
      <div className="mt-4 pt-3 border-t border-border/30 flex flex-col items-center gap-0.5 opacity-50">
        <p className="text-sm font-bold tracking-tight text-gold-gradient">Viciss AIOS</p>
  ...
  const content = (
    <>
      ...
      >
        ...
        {/* Hidden file input — label-wrapping ensures reliable mobile tap */}
        <input
        ...
        <div className="w-full z-10 flex flex-col space-y-4 pb-8 sm:pb-4">
          ...
          <div className="glass w-full rounded-3xl sm:rounded-2xl p-4 sm:p-5 pt-[5.5rem] sm:pt-[6.5rem] flex flex-col relative">
              ...
              {editingProfile ? (
                <div className="space-y-3 max-w-[260px] mx-auto">
                  ...
                  <Input
                    ...
                    className="bg-secondary/50 border-border/60 h-9 text-center text-xs"
                  />
            ...
            {/* Settings Tab Switcher — pill style like bottom nav */}
            <div className="flex items-stretch rounded-full overflow-hidden floating-nav-pill w-full px-1 py-0.5 shadow-[0_4px_16px_rgba(0,0,0,0.4)] border border-white/10">
              {tabs.map(tab => {
                ...
                return (
                  ...
                  >
                    <div className={`tg-tab-icon-wrap ${isActive ? "tg-tab-active" : ""}`}>
                      {tab.icon}
                    </div>
                    <span className={`tg-tab-label ${isActive ? "tg-label-active" : ""}`}>{tab.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          {/* Tab Content */}
          {activeTab === "rabbit" && renderRabbitTab()}
